...
$RealmName = Get-EnvOrDefault -Name 'REALM_NAME' -DefaultValue 'EventfindR'
$ClientId = Get-EnvOrDefault -Name 'CLIENT_ID' -DefaultValue 'eventfindr-client'
$PreferredClientSecret = Get-EnvOrDefault -Name 'CLIENT_SECRET' -DefaultValue 'eventfindr-client-secret'
$ForceRecreateRealm = Get-BoolEnvOrDefault -Name 'FORCE_RECREATE_REALM' -DefaultValue $true

$Roles = @('ADMIN', 'ORGANIZER', 'USER')
$Users = @(
    @{ Username = 'admin@eventfindr.sk'; Password = 'admin123'; Role = 'ADMIN'; FirstName = 'Admin'; LastName = 'EventfindR' },
    @{ Username = 'organizer@eventfindr.sk'; Password = 'organizer123'; Role = 'ORGANIZER'; FirstName = 'Event'; LastName = 'Organizer' },
    @{ Username = 'user@eventfindr.sk'; Password = 'user123'; Role = 'USER'; FirstName = 'Jano'; LastName = 'Novak' }
)
...
function Invoke-ApiPost {
    ...
    Invoke-WebRequest -UseBasicParsing -Method Post -Uri "$KeycloakUrl$Path" -Headers (Get-AuthHeaders) -ContentType 'application/json' -Body (ConvertTo-JsonBody -Payload $Payload) | Out-Null
}
...
function Invoke-ApiPut {
    ...
    Invoke-WebRequest -UseBasicParsing -Method Put -Uri "$KeycloakUrl$Path" -Headers (Get-AuthHeaders) -ContentType 'application/json' -Body (ConvertTo-JsonBody -Payload $Payload) | Out-Null
}
...
function Invoke-ApiDelete {
    ...
    if ($null -ne $Payload) {
        Invoke-WebRequest -UseBasicParsing -Method Delete -Uri "$KeycloakUrl$Path" -Headers (Get-AuthHeaders) -ContentType 'application/json' -Body (ConvertTo-JsonBody -Payload $Payload) | Out-Null
    } else {
        Invoke-WebRequest -UseBasicParsing -Method Delete -Uri "$KeycloakUrl$Path" -Headers (Get-AuthHeaders) | Out-Null
    }
}
...
function Resolve-Email {
    ...
    return "$Username@eventfindr.local"
}
...
function Print-Summary {
    Write-Host ''
    Write-Host '========================================='
    Write-Host '  EventfindR Keycloak Setup Complete!'
    Write-Host '========================================='
    Write-Host ''
    ...
    Write-Host '  - admin@eventfindr.sk / admin123 / ADMIN'
    Write-Host '  - organizer@eventfindr.sk / organizer123 / ORGANIZER'
    Write-Host '  - user@eventfindr.sk / user123 / USER'
    Write-Host ''
    Write-Host 'Keycloak admin console: http://localhost:8081'
    Write-Host ''
}
